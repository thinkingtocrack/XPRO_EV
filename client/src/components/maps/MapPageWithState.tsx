...
const MapPageWithState = memo(({stations,setStations}:{stations:null|{location:{coordinates:[number,number]},_id:string}[],setStations:(stations:{location:{coordinates:[number,number]},_id:string}[])=>void}) => {
    ...
    const getCoordinatesData = async (data:MapCameraChangedEvent)=>{
      try {
        ...
        const result = await api.get('/api/user/charger/within',{
          params:bounds
        })
    ...
    return (
      <>
        <MapProvider>
          <Map restriction={{latLngBounds:{north:43.557112718774285,east:114.41786831229079,south:-10.106144163314845,west:49.309998626033675}}} zoomControl onBoundsChanged={getStationData} {...cameraProps} onCameraChanged={handleCameraChange}  mapId={false?env.VITE_GOOGLE_MAP_ID:'49ae42fed52588c3'}  reuseMaps={true}  disableDefaultUI>
  ...
  export default MapPageWithState